...
text_color = "#FFFFFF"  # Material blue-grey 100
mapbox_land_color = "#0078AC"
...
mappings["sex"] = {-1: "All genders", 0: "Males", 1: "Females"}
...
def set_projection_bounds(df_d):
    ...
    data_3857 = ([df_d.x.min(), df_d.y.min()], [df_d.x.max(), df_d.y.max()])
    data_center_3857 = [
        [
...
def build_query(selections, exclude=None):
    ...
    """
    other_selected = {
        sel for c, sel in selections.items() if (c != exclude and sel != -1)
    }
    if other_selected:
...
def build_datashader_plot(
    ...
    cvs = ds.Canvas(
        plot_width=1400, plot_height=1400, x_range=x_range, y_range=y_range
    )
...
def build_histogram_default_bins(
    df, column, selections, orientation, colorscale_name, colorscale_transform
):
    """
...
def build_updated_figures(
    ...
    if dragmode:
        ...
        position = {
            ...
            "bearing": 0,
            "center": {
                "lon": data_center_4326[0][0] - 100,
                "lat": data_center_4326[0][1] - 10,
            },
        }
...
def register_update_plots_callback(client):
    ...
    @app.callback(
        ...
        [
            ...
            Input("education-histogram", "selectedData"),
            Input("income-histogram", "selectedData"),
            Input("cow-histogram", "selectedData"),
